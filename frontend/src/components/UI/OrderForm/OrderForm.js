...
const OrderForm = () => {
  ...
  return (
    <div className="flex flex-col mt-2.5">
      <Input placeholder="Имя" value={values.name} onChange={(e) => setValues({...values, name: e.target.value})} className="mb-2"/>
      <Input placeholder="Номер телефона" value={values.phone} onChange={(e) => setValues({...values, phone: e.target.value})} className="mb-2"/>
      <div className="relative">
        <Input placeholder="Адрес доставки" value={values.address} onChange={(e) => addressHandler(e.target.value)}  className="mb-2 absolute left-0 right-0 z-10"/>
        {tip &&
        ...
        <div className="transition-all cursor-pointer hover:translate-x-1" onClick={() => setTipAsValue()}>
          {tip}
        </div>
      </div>}
      </div>
